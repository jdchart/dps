...
def merge_json_segments(input_directory, output_directory):
    ...
    pattern = re.compile(r"(LMI_\d+)_(\d+)_(\d+(\.\d+)?)_(\d+(\.\d+)?)_(\d+)_(\d+)\.json$")
    
    # Recherche de tous les fichiers JSON dans le répertoire
    json_files = glob(os.path.join(input_directory, "*.json"))
    
    # Organisation des fichiers par identifiant unique (date + ensemble + numéro de vidéo)
    video_segments = {}
    for file_path in json_files:
        filename = os.path.basename(file_path)
        match = pattern.search(filename)
        if match:
            date = match.group(1)                  # Identifiant de date (e.g., LMI_20230428)
            ensemble = int(match.group(2))         # Nouvelle section 'ensemble'
            start_time = float(match.group(3))     # Temps de début du segment (e.g., 600)
            end_time = float(match.group(5))       # Temps de fin du segment (e.g., 833.833)
            video_number = int(match.group(7))     # Numéro de vidéo (e.g., 1)
            segment = int(match.group(8))          # Segment number

            # Identifiant unique prenant en compte la date, l'ensemble et le numéro de vidéo
            video_id = f"{date}_{ensemble}_{video_number}"
    ...
    for video_id, segments in video_segments.items():
        ...
        end_time_global = segments[-1][2]  # Initialisé avec la fin du dernier segment
