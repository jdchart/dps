...
def merge_json_segments(input_directory, output_directory):
    ...
    pattern = re.compile(r"(LMI_\d+)_(\d+(\.\d+)?)_(\d+(\.\d+)?)_(\d+)_(\d+)\.json$")
    ...
    for file_path in json_files:
        ...
        if match:
            ...
            end_time = float(match.group(4))        # Temps de fin du segment (e.g., 833.833)
            video_number = int(match.group(6))      # Numéro de vidéo (e.g., 1)
            segment = int(match.group(7))  
    ...
    for video_id, segments in video_segments.items():
        ...
        end_time_global = segments[-1][2]  # Initialisé avec le début du dernier segment
